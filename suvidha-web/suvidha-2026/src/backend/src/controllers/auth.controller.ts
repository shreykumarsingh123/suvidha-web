...
export const sendOtp = async (req: Request, res: Response) => {
  ...
  await client.messages.create({
    body: `Your Suvidha OTP: ${otp}. Expires in 10 min.`,
...
export const verifyOtp = async (req: Request, res: Response) => {
  ...
  if (action === 'signup' && user) return res.status(400).json({ message: 'User exists' });
  if (action === 'login' && !user) return res.status(400).json({ message: 'User not found' });
