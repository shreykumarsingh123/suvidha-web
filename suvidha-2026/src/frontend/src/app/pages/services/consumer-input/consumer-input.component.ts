        ...
        <div class="w-full h-full flex items-center justify-center pt-28 pb-10">
            <div class="max-w-lg w-full bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-enter">
                ...
                <div class="p-8">
                    ...
                    <!-- Mini Keypad (Larger Keys like index-11.html) -->
                    <div class="grid grid-cols-3 gap-4 mb-8">
                        <button *ngFor="let num of [1,2,3,4,5,6,7,8,9]" 
                                (click)="addKey(num.toString())" 
                                class="btn-tactile h-20 bg-white text-slate-700 text-3xl font-bold rounded-xl border-b-[6px] border-slate-200 shadow-sm flex items-center justify-center active:border-b-0 active:translate-y-2 transition-all cursor-pointer">
                            {{num}}
                        </button>
                        <button class="btn-tactile h-20 bg-amber-50 text-amber-700 font-bold rounded-xl border-b-[6px] border-amber-200 shadow-sm flex items-center justify-center active:border-b-0 active:translate-y-2 transition-all cursor-pointer text-lg" (click)="clearKey()">CLR</button>
                    ...
                    <button (click)="fetchBill()" class="w-full py-5 bg-blue-600 text-white text-xl font-bold rounded-xl shadow-lg border-b-[6px] border-blue-800 active:border-b-0 active:translate-y-1.5 transition-all flex items-center justify-center gap-3 cursor-pointer">
